...
import { Auth, AccountAndBillings } from '@/common/constants'
  ...
  methods: {
    ...
    getTimeExpired() {
      this.intervalid = setInterval(() => {
        ...
        this.changes = coundownTimer(AccountAndBillings.LIFE_TIME_OF_CODE - (new Date().getTime() - new Date(this.timeExpired).getTime()))
      }, 1000)
    },
    ...
    onConfirmTelCode() {
      memberAccountServices.verifyTelCode(this.confimTelCode).then((response) => {
        if (response.data) {
