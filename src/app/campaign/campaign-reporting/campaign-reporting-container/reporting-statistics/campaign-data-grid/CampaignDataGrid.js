...
import { campaignTotal } from '@/mock/data/campaignReporting'
...
export default {
  ...
  data() {
    return {
      ...
      listCampaignInfoColumns: CampaignReporting.listCampaignInfoColumns
    }
  },
  watch: {
    treeData: {
  ...
  methods: {
    ...
    addTotalRowData(data) {
      campaignTotal.forEach(totalRow => {
        data.push(totalRow)
      })
      return data
    },
    ...
    getListStatus(status) {
      ...
      switch (status) {
        case this.campaignStatus.STANDBY:
          listStatus = [this.campaignStatus.ENABLED, this.campaignStatus.PAUSED]
          break
        case this.campaignStatus.ENABLED:
    ...
    getCurrencyType() {
      ...
      switch (this.language) {
        case this.commonConstant.KO_CODE:
          currencyType = this.commonConstant.CURRENCY.KOREA_TYPE
          break
        case this.commonConstant.EN_CODE:
          currencyType = this.commonConstant.CURRENCY.USA_TYPE
          break
        default:
          currencyType = this.commonConstant.CURRENCY.JAPAN_TYPE
          break
      }
      return currencyType
    },
