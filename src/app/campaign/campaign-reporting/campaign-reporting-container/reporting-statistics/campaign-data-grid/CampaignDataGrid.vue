  ...
  <el-table
    ...
    <template v-if="columns.length">
      <template v-for="column in columns">
        ...
        <el-table-column v-else-if="column.isCampaigns" :key="column.prop" v-bind="column">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.parent" :class="{'campaigns-parent-row': scope.row.parent }"> A </span>
          </template>
        </el-table-column>

        <el-table-column v-else-if="column.isPrice" :key="column.prop" v-bind="column">
          <template slot-scope="scope">
            <currency-input
              v-if="scope.row.parent"
              :type="'bidPrice'"
              :input-value="data[scope.$index].bidPrice"
              :row-index="scope.$index"
              :currency-type="getCurrencyType()"
              :change-value="onChangeBudget" />
            <div v-else-if="scope.row.bidPrice" class="status-area">
