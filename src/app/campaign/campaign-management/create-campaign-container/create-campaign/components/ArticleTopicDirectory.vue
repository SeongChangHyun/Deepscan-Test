      ...
      <div class="container-tree">
        ...
        >
          <span slot-scope="{ node, data  }" class="custom-tree-node">
            ...
            <span>
              <svg-icon icon-class="close-button" class="close-btn" @click="delItem({node, data})"/>
            </span>
          </span>
        </el-tree>
      </div>
...
export default {
  ...
  computed: {
    ...
    data() {
      return this.processData(this.listArticleTopicDirectory)
    },
    dataTreeSelected() {
      return this.processDataSelect(this.data, this.dataCreateCampaign.article_topics)
    }
  },
  created() {
    this.$store.dispatch('campaign/loadListArticleTopicDirectory', {
  ...
  methods: {
    genI18n(params) {
      return this.$t(
        `createCampaign.stepCreateCampaign.articleTopicDirectory.${params}`
      )
    },
    changeData() {
      const data = this.$refs.UserInterestDirTree.getCheckedKeys()
      this.dataCreateCampaign.article_topics = data
    },
    delItem({ data, node }) {
      this.$refs.UserInterestDirTree.setChecked(node.key, false, true)
      this.dataCreateCampaign.article_topics = this.$refs.UserInterestDirTree.getCheckedKeys()
    },
