        ...
        <el-row>
          ...
          <el-col :span="18">
            <el-radio-group v-model="value">
              <el-radio label="all">{{ genI18n('allUser') }}</el-radio>
              <el-radio label="custom">{{ genI18n('specificUserGroup') }}</el-radio>
            </el-radio-group>
...
export default {
    ...
    listLocation() {
      return this.locations
        ...
        .map(v => {
          ...
          v.status = { ...this.locationSelected[index] }.status || CampaignManagementConstants.TARGETING_TYPE.DEFAULT
          return v
        })
    }
  },
  methods: {
    ...
    selectLocationTarget(item) {
      this.locationSelected.push({ ...item, status: CampaignManagementConstants.TARGETING_TYPE.TARGETED })
    },
    selectLocationExclude(item) {
      this.locationSelected.push({ ...item, status: CampaignManagementConstants.TARGETING_TYPE.EXCLUDED })
    },
    deleteLocation(item) {
